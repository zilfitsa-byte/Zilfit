...
def require_confidence(data: dict, key: str) -> float:
    value = data.get(key)
...
def main() -> None:
    if len(sys.argv) != 2:
        print("Usage: python3 runtime/run_z_ux_handoff_from_json.py <input.json>")
        sys.exit(1)
    ...
    runtime_packet = build_z_ux_runtime_packet(
        ...
        confidence=require_confidence(data, "confidence"),
