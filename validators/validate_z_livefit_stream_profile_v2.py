"""
ZILFIT LiveFit Scan Stream Profile Validator v2
...
def major_quality_fail(profile):
    """
    Returns list of major quality failures that force rejection
    regardless of declared scan_confidence.
    """
    failures = []
...
def expected_decision(profile):
    """
    Compute expected decision_status from profile fields.
    ...
    if mq_fails:
        return "rejected", f"major quality failures: {mq_fails}"

    confidence = profile.get("scan_confidence", 0.0)
    if confidence >= 0.85:
        return "pass", None
    elif confidence >= 0.75:
        return "review_required", f"scan_confidence {confidence} in review band"
    else:
...
def validate(path):
    ...
    # Boolean fields
    for bool_field in ("stream_ready", "foot_detected", "floor_plane_anchor",
    ...
    for score_field in ("motion_blur_score", "scale_anchor_confidence",
                        "frame_consistency_score", "scan_confidence",
                        "estimated_arch_index"):
    ...
    exp, reason = expected_decision(profile)
    declared = profile.get("decision_status")

    if declared != exp:
        fail(
            "decision_inconsistency",
            [f"expected decision_status={exp} ({reason}), got {declared}"]
        )

    notes = []
    if reason:
        notes.append(reason)
    ok(notes if notes else None)


if __name__ == "__main__":
    if len(sys.argv) != 2:
        fail("Usage: python3 validate_z_livefit_stream_profile_v2.py <profile.json>")
    validate(sys.argv[1])
